...
so the page keeps working when the network doesn't. Three pieces are exceptions
and say so on their own pages: `weather` is a live forecast, `artworks` fetches
every picture it shows from the museum as it shows it, and `reading` pulls a
whole book from Standard Ebooks each time it runs.
